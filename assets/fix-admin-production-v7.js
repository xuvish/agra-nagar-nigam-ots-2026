...
function modal(){
 ...
 m.querySelector('#adminLoginForm').onsubmit=async e=>{e.preventDefault();const u=m.querySelector('#adminUser').value.trim(),p=m.querySelector('#adminPass').value,msg=m.querySelector('#adminLoginMsg');if(u===ADMIN_USER&&await sha256(p)===ADMIN_PASS_SHA256){sessionStorage.setItem(AUTH_KEY,'1');window.__otsAdminCred={username:u,password:p};msg.className='msg show ok';msg.textContent='Access granted.';setTimeout(()=>{close();if(!window.__otsDetailOnlyLogin){ORIGINAL_OPEN();enhance()}window.__otsDetailOnlyLogin=false;document.dispatchEvent(new Event('ots:admin-login'))},120)}else{msg.className='msg show err';msg.textContent='Invalid username or password.'}};
 return m;
}
function enhance(){
 const rm=document.getElementById('reportModal');if(!rm)return;const p=rm.querySelector('.modalbox p');if(p)p.textContent='Select five OTS reports (Applicant, Collection, In-Process, Financial, Zone Summary), or the original seven-report set. Report type is recognized from report content, not filename.';
 ...
 const input=document.getElementById('masterFile');const refresh=()=>{const count=input?.files?.length||0;box.textContent=[5,7].includes(count)?count+' files selected · ready for content recognition and reconciliation.':count+' file(s) selected · choose 5 or 7 OTS reports.';const b=rm.querySelector('.btn.primary');if(b)b.disabled=![5,7].includes(count)};if(input&&!input.dataset.prodAdmin){input.dataset.prodAdmin='1';input.addEventListener('change',refresh)}refresh();
}
...
window.otsUnlockDetail=function(){if(window.__otsAdminCred?.password){document.dispatchEvent(new Event('ots:admin-login'));return}window.__otsDetailOnlyLogin=true;const m=modal();m.classList.add('on');setTimeout(()=>m.querySelector('#adminUser')?.focus(),50)};
