...
function workflow(E){
 ...
 const apps=new Map((E.apps||[]).map(a=>[appNo(a),a]).filter(([k])=>k));
 const roster=window.PUBLIC?.roster||[],master=window.OTS_MASTER_WARDS||{};
...
function aggregate(E){
 ...
 return{snapshot:S(E.snapshot),city,zones,cityDaily:toDaily(cityDaily),zoneDaily:Object.fromEntries(ZONES.map(z=>[z,toDaily(zoneDaily[z])])),cashiers:toCash(cash),cashiersToday:toCash(todayCash),zoneCashiers:zc,paymentMetrics:pm,workflowCity:wf.city,stageByZone:wf.byZone,workflowUnallocated:wf.unallocated,onlineCity,onlineByZone,riRows:E.riRows||[],wardRows:E.wardRows||[],meta:{sourceFiles:E.files||[],engine:'production-v9',collectionAudit:E.collectionAuthorityAudit||null,sourceAuthority:E.sourceAuthorityAudit||null,classifierAudit:E.classifierAudit||null,masterReceiptAudit:E.masterReceiptAudit||null,unallocated:{applications:N(city.applications)-zoneSum('applications'),approved:N(city.approved)-zoneSum('approved'),inProcess:N(city.inProcess)-zoneSum('inProcess'),collection:round(N(city.collection)-collectionSum),receipts:N(city.receipts)-receiptSum},propertyUnallocated:{collection:round(N(city.collection)-wardCollection)},paymentExportDifference:round(paidExport-N(city.collection))}};
}
