...
function zoneCard(z){const c=control(z),m=collection(z),o=online(z),d=done(z);return'<div class="prod-zone" data-zone="'+z+'"><h4>'+z+'</h4><span class="officer">'+esc(TS[z])+'</span><div class="prod-zone-grid"><div class="prod-zstat" data-zone-metric="applications"><span>Applications</span><b>'+number(c.applications)+'</b></div><div class="prod-zstat" data-zone-metric="approved"><span>Approved</span><b>'+number(c.approved)+'</b></div><div class="prod-zstat" data-zone-metric="inprocess"><span>In Process</span><b>'+number(c.inProcess)+'</b></div><div class="prod-zstat" data-zone-metric="rejected"><span>Rejected</span><b>'+number(c.rejected)+'</b></div><div class="prod-zstat" data-zone-metric="paymentstarted"><span>Payment Started</span><b>'+number(started(z))+'</b></div><div class="prod-zstat" data-zone-metric="paymentdone"><span>Payment Done</span><b>'+(d==null?'—':number(d))+'</b></div><div class="prod-zstat" data-zone-metric="collection"><span>Total Collection</span><b>'+money(m.total)+'</b></div><div class="prod-zstat today-line" data-zone-metric="today"><span>Today Collection</span><b>'+money(m.today)+' · '+number(m.todayReceipts)+' receipts</b></div><div class="prod-zstat online-line" data-online="'+z+'"><span>Online Paid</span><b>'+money(o.amount)+' · '+number(o.receipts)+' receipts</b></div></div><span class="prod-zview">View details →</span></div>'}
function zoneTable(){
