...
async function showApplications(kind,z='All'){
 openModal(kind==='approved'?'Approved Applications':kind==='inprocess'?'In Process Applications':'Total Applications',z,'<div class="prod-modal-empty">Loading saved application details…</div>');
...
function init(){if(state.ready)return;state.ready=true;ensure();document.addEventListener('click',e=>{const root=e.target.closest?.('#productionDashboard');if(!root)return;const online=e.target.closest('[data-online]'),card=e.target.closest('[data-key]'),stage=e.target.closest('[data-stage]'),zone=e.target.closest('.prod-zone');if(!(online||card||stage||zone))return;e.preventDefault();e.stopImmediatePropagation();if(online)showOnline(online.dataset.online);else if(card)openKey(card.dataset.key);else if(stage){if(stage.dataset.stage==='Total In Process')showApplications('inprocess',stage.dataset.zone);else showStage(stage.dataset.stage,stage.dataset.zone)}else showZone(zone.dataset.zone)},{capture:true});render();setTimeout(()=>releaseInitialLoading(true),45000);setInterval(tick,1000);document.addEventListener('ots:shared-applied',()=>{if(!state.history)render()});document.addEventListener('ots:shared-ready',()=>{if(!state.history)render()});document.addEventListener('ots:detail-ready',()=>{if(!state.history)render()});document.addEventListener('ots:published',()=>{state.history=null;state.historyMeta=null;state.historyDetail=null;render()})}
