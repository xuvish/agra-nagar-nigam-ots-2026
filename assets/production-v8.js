...
function collectionDate(){if(state.history)return S(state.history.snapshot);const p=Object.fromEntries(new Intl.DateTimeFormat('en-CA',{timeZone:'Asia/Kolkata',year:'numeric',month:'2-digit',day:'2-digit'}).formatToParts(new Date()).map(x=>[x.type,x.value]));return `${p.year}-${p.month}-${p.day}`}
function collection(z='All'){const rows=daily(z),snap=S(payload()?.snapshot),todayDate=collectionDate(),today=rows.find(r=>S(r.date)===todayDate)||{};return{daily:rows,total:rows.reduce((a,r)=>a+N(r.amount),0),receipts:rows.reduce((a,r)=>a+N(r.receipts),0),today:N(today.amount),todayReceipts:N(today.receipts),todayDate,asof:snap}}
...
function status(){
 const p=payload(),strip=$('prodLiveStrip');if(!strip)return;const has=!!S(p?.snapshot),mode=state.history?'history':has?'live':'empty';strip.dataset.mode=mode;const updated=state.historyMeta?.updated_at||(!state.history?window.__otsSharedState?.updatedAt:null);
 ...
 const d=$('prodDateV8');if(d){d.querySelector('small').textContent=state.history?(state.historyMeta?.is_final?'Historical · Daily Final':'Historical report'):'Today · live time';$('prodClockV8').textContent=state.history?(fmtDate(p.snapshot)+(state.historyMeta?.is_final?' · Daily Final':' · Saved report')):clock()}
}
...
function pendingGrid(z='All'){
 const s=stageStats(z);const items=[['With Applicant',s['With Applicant'],'Correction pending'],['CTO',s.CTO,'Zone / Ward / RI marking'],['RI',s.RI,'Application checking'],['TS',s.TS,'Final approval'],['IT',s.IT,'Gaurav Sinha']];if(s.Other+s.gap)items.push(['Needs Review',s.Other+s.gap,'Source ward evidence needed']);items.push(['Total In Process',s.total,'Reconciled total']);
...
function render(){
 ...
 host.querySelectorAll('.prod-zone').forEach(el=>el.onclick=e=>{showZone(el.dataset.zone)});
 host.querySelectorAll('[data-online]:not(.prod-zone [data-online])').forEach(el=>el.onclick=e=>{e.stopPropagation();showOnline(el.dataset.online)});
...
let modalParentZone=null;
function closeModal(){if(modalParentZone){const z=modalParentZone;modalParentZone=null;showZone(z);return}$('prodModal')?.classList.remove('open');document.body.classList.remove('prod-modal-lock')}
function openModal(title,sub,html,pdfFn,pdfLabel='Export PDF'){ensureModal();const oldTitle=$('prodModalTitle').textContent;const parent=ZONES.find(z=>oldTitle===z+' Zone');if($('prodModal').classList.contains('open')&&parent&&!ZONES.some(z=>title===z+' Zone'))modalParentZone=parent;$('prodModalTitle').textContent=title;$('prodModalSub').textContent=sub||'';$('prodModalBody').innerHTML=html;const a=$('prodModalActions');a.innerHTML='';if(pdfFn){const b=document.createElement('button');b.textContent=pdfLabel;b.onclick=pdfFn;a.appendChild(b)}$('prodModal').classList.add('open');document.body.classList.add('prod-modal-lock')}
...
async function detailFor(date=S(payload()?.snapshot)){if(state.history&&state.historyDetail&&state.historyDetail.snapshot===date)return state.historyDetail;const remote=await window.OTS_PRIVATE?.load?.(date);if(remote)return remote;if(state.history)return await window.OTS_AUTHORITY?.loadDetail?.(date);return window.OTS_AUTHORITY?.detail?.()||await window.OTS_AUTHORITY?.restoreDetail?.(date)}
...
async function showApplications(kind,z='All'){
 ...
 if(rows.length)html+='<h3 class="prod-section-title">Application Detail</h3>'+table([{key:'app',label:'Application'},{key:'name',label:'Applicant / Owner'},{key:'mobile',label:'Mobile'},{key:'address',label:'Address'},{key:'uid',label:'Property UID'},{key:'zone',label:'Zone'},{key:'ward',label:'Ward'},{key:'ri',label:'RI / TC'},{key:'status',label:'Status'}],rows.map(a=>({app:esc(appNo(a)),name:esc(appName(a)),mobile:phone(mobileOf(a)),address:esc(a['Property address']||a.Address||'—'),uid:esc(a['Property UID']||'—'),zone:esc(zoneOf(a)||'—'),ward:esc(wardOf(a)||'—'),ri:esc(riOf(a)||'—'),status:esc(a['Application status']||'—')})));
...
async function showStage(stage,z='All'){
 ...
 rows=rows.filter(a=>{const p=L(a['Pending with']);if(stage==='With Applicant')return p.includes('applicant')||p.startsWith('respected');if(stage==='IT')return /it officer|gaurav sinha/.test(p);if(stage==='CTO')return p.includes('cto');if(stage==='RI')return /\bri\b/.test(p)||p.includes('(ri)');if(stage==='TS')return /\bts\b/.test(p)||p.includes('(ts)');if(isOther)return !/it officer|gaurav sinha/.test(p)&&!p.includes('applicant')&&!p.includes('cto')&&!/\bri\b/.test(p)&&!p.includes('(ri)')&&!/\bts\b/.test(p)&&!p.includes('(ts)');return false});
...
async function showOnline(z='All'){const o=online(z),D=await detailFor(),apps=D?.apps||[],payments=D?.payments||[],amap=new Map(apps.map(a=>[appNo(a),a]));const rows=payments.filter(p=>L(p['Cashier / channel'])==='online'&&(z==='All'||S(p['Property zone']||p['Allocated zone'])===z));let html=summary([['Online Collection',money(o.amount)],['Online Receipts',number(o.receipts)],['Applicants',number(o.applicants)],['Zone',z]]);if(rows.length)html+='<h3 class="prod-section-title">Online Paid Applicant / Receipt Detail</h3>'+table([{key:'app',label:'Application'},{key:'name',label:'Applicant / Owner'},{key:'mobile',label:'Mobile'},{key:'receipt',label:'Receipt'},{key:'date',label:'Date'},{key:'zone',label:'Zone'},{key:'ward',label:'Ward'},{key:'ri',label:'RI / TC'},{key:'amount',label:'Amount',num:true}],rows.slice(0,1500).map(p=>{const a=amap.get(appNo(p))||{};return{app:esc(appNo(p)||'—'),name:esc(appName(a)||p['Payer name']||'—'),mobile:phone(mobileOf(a)||p['Mobile (masked)']),receipt:esc(p['Receipt number']||'—'),date:esc(fmtDate(p['Payment date'])),zone:esc(S(p['Property zone']||p['Allocated zone'])||'—'),ward:esc(wardOf(a)||p['Property ward']||p['Allocated ward']||'—'),ri:esc(riOf(a)||p['Property RI']||p['Allocated RI / TC']||'—'),amount:money(p['Amount (INR)'])}}));else html+='<div class="prod-modal-empty">Aggregate online amount is saved. Receipt/applicant detail is private to the browser where the reports were uploaded.</div>';openModal('Online Paid · '+z,'Online receipts and applicant information',html)}
async function showToday(){const m=collection('All'),D=await detailFor(),date=S(m.todayDate),rows=(D?.payments||[]).filter(p=>S(p['Payment date'])===date);const cash=new Map();for(const p of rows){const k=S(p['Cashier / channel'])||'Unknown',x=cash.get(k)||{name:k,receipts:0,amount:0};x.receipts++;x.amount+=N(p['Amount (INR)']);cash.set(k,x)}let html=summary([[fmtDate(date),money(m.today)],['Receipts',number(m.todayReceipts)],['View','All Zones'],['Report Through',fmtDate(payload().snapshot)]]);if(rows.length){html+='<h3 class="prod-section-title">Cashier / Channel</h3>'+table([{key:'name',label:'Cashier / Channel'},{key:'receipts',label:'Receipts',num:true},{key:'amount',label:'Collection',num:true}],[...cash.values()].sort((a,b)=>b.amount-a.amount).map(x=>({name:esc(x.name),receipts:number(x.receipts),amount:money(x.amount)})))+'<h3 class="prod-section-title">Receipts Generated</h3>'+table([{key:'receipt',label:'Receipt'},{key:'app',label:'Application'},{key:'cashier',label:'Cashier / Channel'},{key:'zone',label:'Property Zone'},{key:'amount',label:'Amount',num:true}],rows.map(p=>({receipt:esc(p['Receipt number']||'—'),app:esc(appNo(p)||'—'),cashier:esc(p['Cashier / channel']||'Unknown'),zone:esc(p['Property zone']||p['Allocated zone']||'—'),amount:money(p['Amount (INR)'])})))}else html+='<div class="prod-modal-empty">Receipt-level rows are private to the browser where this date’s reports were uploaded.</div>';openModal('Today Collection','Receipt count, amount and cashier detail for '+fmtDate(date),html)}
async function showCollection(){const m=collection('All'),p=payload(),cash=p.cashiers||[],D=await detailFor(),apps=D?.apps||[],payments=D?.payments||[],byApp=new Map(apps.map(a=>[appNo(a),a]));let cum=0;let html=summary([['Collection',money(m.total)],['Receipts',number(m.receipts)],['Report Through',fmtDate(m.asof)],['Recovery',pct(m.total,control().demand)]])+'<h3 class="prod-section-title">Cashier / Channel</h3>'+table([{key:'name',label:'Cashier / Channel'},{key:'receipts',label:'Receipts',num:true},{key:'amount',label:'Collection',num:true}],cash.map(r=>({name:esc(r.name||r.cashier||'Unknown'),receipts:number(r.receipts),amount:money(r.amount)})))+'<h3 class="prod-section-title">Daily Collection</h3>'+table([{key:'date',label:'Date'},{key:'receipts',label:'Receipts',num:true},{key:'amount',label:'Collection',num:true},{key:'cum',label:'Cumulative',num:true}],m.daily.map(r=>{cum+=N(r.amount);return{date:fmtDate(r.date),receipts:number(r.receipts),amount:money(r.amount),cum:money(cum)}}));if(D?.scope)html+='<div class="prod-modal-empty">Protected receipt list below is limited to '+esc(D.scope)+' applicants; the totals above are citywide.</div>';if(payments.length)html+='<h3 class="prod-section-title">Receipt Detail</h3>'+table([{key:'receipt',label:'Receipt'},{key:'app',label:'Application'},{key:'name',label:'Applicant / Owner'},{key:'mobile',label:'Mobile'},{key:'address',label:'Address'},{key:'zone',label:'Zone'},{key:'ward',label:'Ward'},{key:'date',label:'Date'},{key:'amount',num:true,label:'Amount'}],payments.map(r=>{const a=byApp.get(appNo(r))||{};return{receipt:esc(r['Receipt number']||'—'),app:esc(appNo(r)||'—'),name:esc(appName(a)||r['Payer name']||'—'),mobile:phone(mobileOf(a)||r['Mobile (masked)']),address:esc(a['Property address']||'—'),zone:esc(zoneOf(a)||r['Property zone']||'—'),ward:esc(wardOf(a)||r['Property ward']||'—'),date:esc(fmtDate(r['Payment date'])),amount:money(r['Amount (INR)'])}}));else html+='<div class="prod-modal-empty">Receipt-level rows require protected detail access.</div>';openModal('Total Collection','All Zones · cashier/channel and receipt detail',html)}
async function showZone(z){
 modalParentZone=null;
 ...
 const stageHtml='<div class="prod-stage-grid">'+[['With Applicant',st['With Applicant'],'Correction pending'],['CTO',st.CTO,'Zone / Ward / RI marking'],['RI',st.RI,'Application checking'],['TS',st.TS,'Final approval'],['IT',st.IT,'Gaurav Sinha'],...(st.Other+st.gap?[['Needs Review',st.Other+st.gap,'Source ward evidence needed']]:[]),['Total In Process',st.total,'Reconciled total']].map(x=>'<div class="prod-stage-card '+(x[0]==='Total In Process'?'total':'')+'" data-stage-modal="'+esc(x[0])+'" data-zone="'+z+'"><span>'+esc(x[0])+'</span><b>'+number(x[1])+'</b><small>'+esc(x[2])+'</small></div>').join('')+'</div>';
 ...
 wardData.push({__total:true,ward:'TOTAL',ri:'—',apps:number(wards.reduce((a,r)=>a+N(r.applications),0)),approved:number(wards.reduce((a,r)=>a+N(r.approved),0)),pending:number(wards.reduce((a,r)=>a+N(r.inProcess),0)),collection:money(wards.reduce((a,r)=>a+N(r.collection),0))});
 const html=summary([
  ...
  ['Online Paid',money(o.amount)],['Total Collection',money(m.total)],['Today Collection',money(m.today)],['Today Receipts',number(m.todayReceipts)]
 ])+
 ...
 const actions={'Applications':()=>showApplications('applications',z),'Approved':()=>showApplications('approved',z),'In Process':()=>showApplications('inprocess',z),'Rejected':()=>showApplications('rejected',z),'Payment Done':()=>showPaymentZone('done',z),'Payment Pending':()=>showPaymentZone('pending',z),'Online Paid':()=>showOnline(z)};
 $('prodModalBody').querySelectorAll('.prod-summary .prod-stat').forEach(el=>{const action=actions[el.querySelector('span')?.textContent];if(action){el.style.cursor='pointer';el.tabIndex=0;el.setAttribute('role','button');el.onclick=action;el.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();action()}}}});
 document.querySelectorAll('[data-stage-modal]').forEach(el=>el.onclick=()=>{if(el.dataset.stageModal==='Total In Process')showApplications('inprocess',z);else showStage(el.dataset.stageModal,z)})
}
...
function pendingResponsibilityRows(){
 return ZONES.map(z=>{const s=stageStats(z);return[z,s['With Applicant'],s.CTO,s.RI,s.TS,s.IT,s.Other+(s.gap||0),s.total]})
}
function pendingResponsibilityTableHTML(){
 const rows=pendingResponsibilityRows(),city=stageStats('All');
 return '<table class="compact"><thead><tr><th>Zone</th><th>With Applicant</th><th>CTO</th><th>RI</th><th>TS</th><th>IT · Gaurav Sinha</th><th>Needs Review</th><th>Total In Process</th></tr></thead><tbody>'+
 rows.map(r=>'<tr><td>'+esc(r[0])+'</td>'+r.slice(1).map(v=>'<td class="center">'+number(v)+'</td>').join('')+'</tr>').join('')+
 '</tbody><tfoot><tr class="total-row"><td>TOTAL</td><td class="center">'+number(city['With Applicant'])+'</td><td class="center">'+number(city.CTO)+'</td><td class="center">'+number(city.RI)+'</td><td class="center">'+number(city.TS)+'</td><td class="center">'+number(city.IT)+'</td><td class="center">'+number(city.Other+(city.gap||0))+'</td><td class="center">'+number(city.total)+'</td></tr></tfoot></table>'
}
...
function exportZonePDF(z,c,m,ri,wards,o,st){
 ...
 const stages=[['With Applicant',st['With Applicant']],['CTO',st.CTO],['RI',st.RI],['TS',st.TS],['Needs Review',st.Other+(st.gap||0)],['TOTAL IN PROCESS',st.total]];
