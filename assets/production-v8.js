...
async function detailFor(date=S(payload()?.snapshot)){if(state.history&&state.historyDetail&&state.historyDetail.snapshot===date)return state.historyDetail;if(state.history)return await window.OTS_AUTHORITY?.loadDetail?.(date);return window.OTS_AUTHORITY?.detail?.()||await window.OTS_AUTHORITY?.restoreDetail?.(date)}
