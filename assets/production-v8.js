...
async function showZone(z){const c=control(z),m=collection(z),p=payload(),ri=(p.riRows||[]).filter(r=>r.zone===z),wards=(p.wardRows||[]).filter(r=>r.zone===z),o=online(z),st=stageStats(z);const stageHtml='<div class="prod-stage-grid">'+[['With Applicant',st['With Applicant'],'Correction pending'],['CTO',st.CTO,'Zone / Ward / RI marking'],['RI',st.RI,'Application checking'],['TS',st.TS,'Final approval'],['Other',st.Other+(st.gap||0),st.gap?'Other / mapping gap':'Other workflow'],['Total In Process',st.total,'Reconciled total']].map(x=>'<div class="prod-stage-card '+(x[0]==='Total In Process'?'total':'')+'" data-stage-modal="'+esc(x[0])+'" data-zone="'+z+'"><span>'+esc(x[0])+'</span><b>'+number(x[1])+'</b><small>'+esc(x[2])+'</small></div>').join('')+'</div>';const html=summary([['Applications',number(c.applications)],['Approved',number(c.approved)],['In Process',number(c.inProcess)],['Payment Started',number(started(z))],['Payment Done',done(z)==null?'—':number(done(z))],['Payment Pending',number(pending(z))],['Online Paid',money(o.amount)],['Collection',money(m.total)]])+'<h3 class="prod-section-title">Application Pendency by Stage</h3>'+stageHtml+'<h3 class="prod-section-title">RI / TC Performance</h3>'+table([{key:'ri',label:'RI / TC'},{key:'apps',label:'Apps',num:true},{key:'approved',label:'Approved',num:true},{key:'pending',label:'In Process',num:true},{key:'paid',label:'Payment Started',num:true},{key:'collection',label:'Collection',num:true}],ri.map(r=>({ri:esc(r.ri),apps:number(r.applications),approved:number(r.approved),pending:number(r.inProcess),paid:number(r.paidApplicants),collection:money(r.collection)})))+'<h3 class="prod-section-title">Ward Position</h3>'+table([{key:'ward',label:'Ward'},{key:'ri',label:'RI / TC'},{key:'apps',label:'Apps',num:true},{key:'approved',label:'Approved',num:true},{key:'pending',label:'In Process',num:true},{key:'collection',label:'Collection',num:true}],wards.map(r=>({ward:esc((r.wardNo??'')+' · '+(r.ward||'')),ri:esc(r.ri),apps:number(r.applications),approved:number(r.approved),pending:number(r.inProcess),collection:money(r.collection)})));openModal(z+' Zone','Complete officer view · '+fmtDate(payload().snapshot),html,()=>exportZonePDF(z,c,m,ri,wards,o,st));document.querySelectorAll('[data-stage-modal]').forEach(el=>el.onclick=()=>{if(el.dataset.stageModal==='Total In Process')showApplications('inprocess',z);else showStage(el.dataset.stageModal,z)})}
