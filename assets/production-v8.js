...
async function showZone(z){
 ...
 const stageHtml='<div class="prod-stage-grid">'+[['With Applicant',st['With Applicant'],'Correction pending'],['CTO',st.CTO,'Zone / Ward / RI marking'],['RI',st.RI,'Application checking'],['TS',st.TS,'Final approval'],['IT',st.IT,'Gaurav Sinha'],['Other',st.Other+(st.gap||0),st.gap?'Other / mapping gap':'Other workflow'],['Total In Process',st.total,'Reconciled total']].map(x=>'<div class="prod-stage-card '+(x[0]==='Total In Process'?'total':'')+'" data-stage-modal="'+esc(x[0])+'" data-zone="'+z+'"><span>'+esc(x[0])+'</span><b>'+number(x[1])+'</b><small>'+esc(x[2])+'</small></div>').join('')+'</div>';
