...
function officerCallingZone(a){
  ...
  if(!text)return null;
  if(text.includes('sheetal gupta'))return{zone:'Chhatta',basis:'Final approval / officer evidence · Sheetal Gupta'};
  if(text.includes('akshay kumar'))return{zone:'Hariparwat',basis:'Final approval / officer evidence · Akshay Kumar'};
  if(text.includes('rambabu')){
...
function enrichLocalApps(){
  ...
  const apps=LOCAL.applications.map(a=>{
    ...
    const call=callingAssignment(x);x['Calling zone']=call.zone;x['Calling basis']=call.basis;
    return x;
  });
...
window.renderFollowup=function(){
  ...
  const assigned=all.length-(by.Citywide||0),mobiles=all.filter(a=>mobileOf(a)).length;
  $('followHint').innerHTML=`<b>${num(all.length)}</b> approved applicants have no receipt/payment record. <b>${num(assigned)}</b> are assigned to a Calling Zone using property/UID evidence first, then final TS or unique RI/TC evidence. Chhatta <b>${num(by.Chhatta||0)}</b> · Hariparwat <b>${num(by.Hariparwat||0)}</b> · Tajganj <b>${num(by.Tajganj||0)}</b> · Lohamandi <b>${num(by.Lohamandi||0)}</b>${by.Citywide?` · Citywide <b>${num(by.Citywide)}</b>`:''}. <b>${num(mobiles)}</b> have a contact value from supplied reports.`;
